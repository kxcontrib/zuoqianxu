...
K1(mlgetexpr){
  ...
  K v,L;
  tag=MLGetNext(ml_lp);
  //printf("%s:%d,xt:%d,xi:%d\n","mlgetexpr",tag,x->t,x->i);
  switch (tag){
  case MLTKINT:v=ki(0);R (MLGetInteger32(ml_lp,&v->i))?v:0;
  case MLTKREAL:v=kf(0.);R (MLGetReal64(ml_lp,&v->f))?v:0;
  case MLTKSYM:if(!MLGetSymbol(ml_lp,&s)){R 0;}else {v=ks(s);MLReleaseSymbol(ml_lp,s);R v;}
  case MLTKSTR:if(!MLGetByteString(ml_lp,&s,&len,0)){R 0;}else {v=kpn(s,len);MLReleaseByteString(ml_lp,s,len);R v;}
  case MLTKFUNC:
    if(!MLGetFunction(ml_lp,&s,&len)){
      R 0;
    }else{
      if(!strcmp(s,"List")){
	L=knk(0);
...
K1(mlgetiarray){
  ...
  */
  ...
  MLDisownIntegerArray(ml_lp, data, dims, heads, d);
  R L;
}
...
K1(mlgetfarray){
  ...
  MLDisownRealArray(ml_lp, data, dims, heads, d);	
  R L;
}
...
K1(mlgetilist){
  ...
  L=ktn(KI,length);
  for(i=0;i<length;i++)kI(L)[i]=*(data+i);
  MLDisownIntegerList(ml_lp, data, length);
  R L;
}
...
K2(mlget){
  int pkt;
  int tag=y->i;
  //printf("%s:xt:%d,xi:%d,yt:%d,yi:%d\n","mlget",x->t,x->i,y->t,y->i);
  while((pkt = MLNextPacket(ml_lp)) && pkt != RETURNPKT){
    MLNewPacket(ml_lp);
  }
  if(!pkt){
    ONMLERR;
...
K3(mleval){
  ...
  MLENDPACKET;
  if(!r){ONMLERR;R 0;}
  else if(!MLFlush(ml_lp)){ONMLERR;R 0;}
  else{v=mlget(y,z);}
